    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.items():
            if isinstance(v, Field):
                logging.info(' found mapping k name: %s ==> %s' % (k, v))
                mappings[k] = v
                if v.primary_key:
                    if primaryKey:
                        raise StanderdError('Doplicate primary key for field: %s' % k)
                    primaryKey = v.name or k
                else:
...
class Model(dict, metaclass=ModelMetaClass):
    ...
    async def findAll(cls, where=None, args=None, **kw):
        print(' find objects by where clause. ')
        sql = [cls.__select__]
        if where:
            sql.append('where')
            sql.append(where)
